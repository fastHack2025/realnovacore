...
export default function TeleventeInscription() {
  ...
  return (
    <main className="relative min-h-screen bg-gradient-to-br from-gray-900 to-black text-white px-6 py-16 overflow-hidden">
      ...
      <div className="max-w-xl mx-auto bg-white/10 p-8 rounded-xl border border-white/20 shadow-xl backdrop-blur">
        <h1 className="text-2xl font-bold mb-6 text-center">📝 Inscription — Télévente</h1>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <PaymentIcon
              ...
              label="PayPal"
              value="paypal"
              current={moyenPaiement}
              set={setMoyenPaiement}
            />
            <PaymentIcon
...
function PaymentIcon({ src, label, value, current, set }: any) {
  return (
    <div
      onClick={() => set(value)}
      className={`cursor-pointer border rounded-lg p-2 text-center w-24 h-24 flex flex-col items-center justify-center transition-all duration-200 ${
        current === value ? "border-green-400 bg-white/10" : "border-white/20"
      }`}
    >
